...
def add_plsql_text(sql_text, bind_number=None):
    ret_text = "declare\n"
    if bind_number:
        ret_text += "@bind" + str(bind_number) + "\n"
    else:
        ret_text += "@bind\n"
    ret_text += "begin\n"
    ret_text += "for r in (\n"
...
def main_for_cui():
    ...
    if args.sql:
        ...
        sql_nobinds_text = make_sql_nobinds(sql_text, args.number)
        filename = "q00.sql"
        if args.number:
            filename = "q00_" + str(args.number) + ".sql"
